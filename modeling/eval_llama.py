...
def call_engine(train_prompt, problem, model, tokenizer=None, max_new = 100, short = True):
    '''
    Given a problem, returns the most likely answer determined by the Llama model
    '''
    test_question = "\n" + "Problem: " + problem + "\n" + "Answer: $"
...
def run(llama_path, size='7B', max=-1):
  
    print("Loading llama model...")
    tokenizer = LlamaTokenizer.from_pretrained(llama_path)
    model = LlamaForCausalLM.from_pretrained(llama_path, device_map="auto")
    print("Model loaded.")
    ...
    with open("outputs_answers_llama_{}.txt".format(size), "w+") as f:
